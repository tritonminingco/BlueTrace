...
from app.models.dataset_common import DatasetTides, DatasetSST, DatasetCurrents, DatasetTurbidity
...
async def main() -> None:
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            logger.info(f'    http://localhost:8080/v1/health')
            logger.info("")
            logger.info(f'  curl -H "X-Api-Key: {admin_key}" \\')
            logger.info(f'    "http://localhost:8080/v1/tides?station_id=DEMO001&start=2024-01-01T00:00:00Z&end=2024-12-31T23:59:59Z"')
            logger.info("")
            logger.info("=" * 60)
